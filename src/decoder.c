...
///Please finish functions: anti_zz(), anti_q(), read_BLOCK()
...
# define M_DQT  0xDB
// SOF0, start of frame 0 (baseline)
# define M_SOF0 0xC0
...
u16 read_len (F_BUFFER* buf) {
    ...
    u16 len;
    // the 16 bits define the length of current section
...
u8 get_symbol_len (DHT* DHT_table, F_BUFFER* buf) {
    ...
    // reset pt pointing to root, status = 0, not found leaf yet
    DHT_reset(DHT_table);
    ...
    // We might find a symbol with code length less than 8 (like the high frequency ones)
    // so we have to put the remaining codes back to the front of the buffer
...
i16 get_symbol (u8 len, F_BUFFER* buf) {
    ...
    if (c >> (len - 1))
        return c;
    // MSB is 0, transform to negative (2 ^ len - c)
    else 
...
void anti_zz (BLOCK obj) {
    ...
    // transform AC, DC coefficients from zigzag order back to original order
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            (*res)[i * 8 + j] = obj[zz_table[i][j]];
        }
    }
    // copy the results to obj
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            obj[i * 8 + j] = (*res)[i * 8 + j];
        }
...
void anti_q (BLOCK obj, DQT* quant) {
    ...
    //****Please finish this function****
    // do anti-quantization by multiplying the quantizer
    for (int i = 0; i < 64; ++i) {
        if (quant->precise == 0) {
            // for 8-bit quantizer
...
void upsampling(MCU* obj, JPEG_INFO* info) {
    ...
    // for each of Y, Cb, Cr channel do upsampling
...
BLOCK* read_BLOCK (JPEG_INFO* info, u8 chan_id) {
    ...
    // channel 1 refers to (0, 0), channel 2 and 3 refer to (1, 1)
    ...
    //****Please finish the part of AC ****
    u8 zeros, idx = 1;
    while (idx < 64) {
        symbol_len = get_symbol_len(AC_table, info->fp);
        if (symbol_len == 0x00) {
            // EOB, the rest are zeros
            while (idx < 64) {
                (*obj)[idx++] = 0;
            }
        }
...
void read_APP0 (JPEG_INFO* info) {
    ...
    // length of section is len bytes
    // length of len indicator is 2 bytes
    // length of data is len - 2 bytes
...
void read_SOF0 (JPEG_INFO* info) {
    ...
    for (int i = 0; i < info->n_color; ++i) {
        ...
        read_u8(&id, info->fp);
        // which channel? Y: 1, Cb: 2, Cr: 3 
        info->CHAN[id] = obj;
        read_u8(&(obj->h_factor), info->fp);
        // Y channel has DQT_ID 0, other has DQT_ID 1
        read_u8(&(obj->DQT_ID), info->fp);
        // low 4 bits indicate vertical subsampling factor
        obj->v_factor = obj->h_factor & 0x0F;
        // high 4 bits indicate horizontal subsampling factor
        obj->h_factor = obj->h_factor >> 4;
        obj->N_BLOCK_MCU = (obj->v_factor) * (obj->h_factor);
        // max subsampling factor among all channels
    ...
    // minimum MCUs needed to cover the whole image
    ...
    // total height and width when combining all MCUs (should be larger than the original image)
...
void read_DHT (JPEG_INFO* info) {
    ...
    while (1) {
        ...
        // Read DHT type
        read_u8(&c, info->fp);
        // AC has high 4 bits 1, DC has high 4 bits 0
        // low 4 bits indicate table destination
        if (c >> 4 == TYPE_DC)
        ...
        // Read info
        // traverse each level to count number of leaves
...
void read_SOS (JPEG_INFO* info) {
    ...
    for (int i = 0; i < (len - 4) / 2; ++i) {
        ...
        // which DC_DHT or AC_DHT table this channel should reference
...
int main (int argc, char* argv[]) {
    ...
    // JPEG_INFO* info = read_JPEG_INFO("../Image/monalisa.jpg");
    // RGB* img = read_JPEG_img(info);
    // write_bmp("../Decode/monalisa.bmp", img);
